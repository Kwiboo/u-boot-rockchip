...
#define SDRAM_OFFSET(offset)		(CFG_SYS_SDRAM_BASE + (offset))
...
struct memblock param_parse_optee_mem(void)
{
	...
	if (mem.size)
		gd->pflags |= GD_P_FLG_BL32_ENABLED;
...
{
	return (base + size >= CFG_SYS_SDRAM_BASE + SDRAM_MAX_SIZE) ?
	       (CFG_SYS_SDRAM_BASE + SDRAM_MAX_SIZE - base) : size;
}
