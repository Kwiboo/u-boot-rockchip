...
int rockchip_early_misc_init_r(void)
{
	...
	ret = misc_read(dev, CPU_VERSION_OFFSET, cpu_code, 1);
	if (ret < 0) {
		debug("%s: could not read cpu-version, ret=%d\n", __func__, ret);
		return 0;
	}
	debug("cpu-version: %x (%02x)\n", (cpu_code[0] >> 3) & 0x7, cpu_code[0]);

	ret = misc_read(dev, 0x05, &cpu_code, 2);
	if (ret < 0) {
		debug("%s: could not read serial-number, ret=%d\n", __func__, ret);
		return 0;
	}
	debug("data: %02x %02x\n", cpu_code[0], cpu_code[1]);

	package = ((cpu_code[0] & BIT(0)) << 3) | ((cpu_code[1] & GENMASK(7, 5)) >> 5);
